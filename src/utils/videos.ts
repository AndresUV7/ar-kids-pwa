import { Video } from '../app/models/Video';
export const HIGIENE_VIDEOS : Video[] = [

    
    {
        nombre: "k",
        ruta : "../../ar-kids-pwa/assets/video/alimentacion.mp4",
        checkpoints : [16, 48],
        preguntas : [
            "¿Debemos lavarnos las manos después de ir al baño?",
            "Completa: La comida debe guardarse en recipientes limpios y ....",

        ],
        respuestas1 : [
            "Si",
            "No"
        ],
        respuestas2 : [
            "Tapados",
        ],
        id : "#k"
    },

    {
        nombre: "a",
        ruta : "../../ar-kids-pwa/assets/video/ducha.mp4",
        checkpoints : [17, 94],
        preguntas : [
            "¿Cada que tiempo hay que ducharse?",
            "Completa: Nos recortamos las uñas con el ...",

        ],
        respuestas1 : [
            "Todos los días",
            "Dos veces a la semana"
        ],
        respuestas2 : [
            "Cortauñas",
        ],
        id : "#a"
    },   

    {
        nombre: "b",
        ruta : "../../ar-kids-pwa/assets/video/higiene general.mp4",
        checkpoints : [38, 131],
        preguntas : [
            "¿Cuál es el mejor lugar para empezar con nuestra higiene personal?",
            "Completa: La higiene personal es....",

        ],
        respuestas1 : [
            "Manos",
            "Pies"
        ],
        respuestas2 : [
            "Fácil"
        ],
        id : "#b"
    },     

    {
        nombre: "c",
        ruta : "../../ar-kids-pwa/assets/video/manos.mp4",
        checkpoints : [11, 76],
        preguntas : [
            "¿Con qué nos lavamos las manos?",
            "Completa: Para secarnos las manos, usamos una ....",

        ],
        respuestas1 : [
            "Solo con agua",
            "Con agua y jabón"
        ],
        respuestas2 : [
            "Toalla"
        ],
        id: "#c"
    },   
    
    {
        nombre: "d",
        ruta : "../../ar-kids-pwa/assets/video/dientes.mp4",
        checkpoints : [14, 85],
        preguntas : [
            "¿Cuantas veces al día debemos cepillar nuestros dientes?",
            "Completa: Despúes de cepillar los dientes, cepillamos la....",

        ],
        respuestas1 : [
            "1",
            "2"
        ],
        respuestas2 : [
            "Lengua",
        ],
        id : "#d"
    },    

    {
        nombre: "f",
        ruta : "../../ar-kids-pwa/assets/video/primeros auxilios.mp4",
        checkpoints : [67, 75],
        preguntas : [
            "¿Cuantos minutos debemos colocar hielo si nos golpeamos?",
            "Completa: La heridas se deben lavar con ....",

        ],
        respuestas1 : [
            "10",
            "5"
        ],
        respuestas2 : [
            "Agua"
        ],
        id : "#f"
    },   
]

export const VESTIR_VIDEOS : Video[] = [
    {
        nombre: "Prendas",
        ruta : "../../ar-kids-pwa/assets/video/prendas.mp4",
        checkpoints : [36, 133],
        preguntas : [
            "¿En que parte del cuerpo va esta prenda de vestir",
            "Completa: Esta prenda de vestir se llama .... "
        ],
        respuestas1 : [
            "Cuello",
            "Cintura"
        ],
        respuestas2 : [
            "Vestido",
        ]
    },    
    
    {
        nombre: "Tender",
        ruta : "../../ar-kids-pwa/assets/video/tender ropa.mp4",
        checkpoints : [31, 67],
        preguntas : [
            "¿Que pinzas dejan menos marcas?",
            "Completa: En los pantalones colocamos las pinzas en la ....",

        ],
        respuestas1 : [
            "Pinzas de plástico",
            "Pinzas de madera"
        ],
        respuestas2 : [
            "Cintura",
        ]
    },

    {
        nombre: "Planchar",
        ruta : "../../ar-kids-pwa/assets/video/planchar.mp4",
        checkpoints : [48, 94],
        preguntas : [
            "¿Que parte de la camisa se plancha primero?",
            "Completa: Cuando planchamos debemos tener cuidado con los ...",

        ],
        respuestas1 : [
            "Cuello",
            "Puños"
        ],
        respuestas2 : [
            "Botones",
        ]
    },   

    {
        nombre: "Lavar",
        ruta : "../../ar-kids-pwa/assets/video/lavar.mp4",
        checkpoints : [27, 91],
        preguntas : [
            "¿Como separamos la ropa para lavar?",
            "Completa: Dejamos la ropa a remojar por el tiempo de una ....",

        ],
        respuestas1 : [
            "Por tamaño",
            "Por color"
        ],
        respuestas2 : [
            "Hora"
        ]
    },     

    {
        nombre: "Doblar",
        ruta : "../../ar-kids-pwa/assets/video/doblar.mp4",
        checkpoints : [110, 133],
        preguntas : [
            "¿En cuántos trozos doblamos la camiseta de adulto",
            "Completa: Doblamos la ropa para tener nuestro armario más ....",

        ],
        respuestas1 : [
            "3",
            "4"
        ],
        respuestas2 : [
            "Ordenado"
        ]
    },        

    {
        nombre: "Cordones",
        ruta : "../../ar-kids-pwa/assets/video/cordones.mp4",
        checkpoints : [26, 44],
        preguntas : [
            "¿Con qué dedos sujetamos el cordón?",
            "Completa: En este último paso, apretamos los ....",

        ],
        respuestas1 : [
            "Pulgar y meñique",
            "Pulgar e índice"
        ],
        respuestas2 : [
            "Cordones"
        ]
    },   
]

export const COCINA_VIDEOS : Video[] = [
    {
        nombre: "Batido",
        ruta : "../../ar-kids-pwa/assets/video/batido.mp4",
        checkpoints : [36, 71],
        preguntas : [
            "¿Cuántas cucharadas de miel se pone en el batido?",
            "Completa: Este batido puedo prepararlo para mi .... "
        ],
        respuestas1 : [
            "2",
            "1"
        ],
        respuestas2 : [
            "Desayuno",
        ]
    },    
    
    {
        nombre: "Ensalada",
        ruta : "../../ar-kids-pwa/assets/video/ensalada fruta.mp4",
        checkpoints : [87, 187],
        preguntas : [
            "¿Que quitamos de las frutas?",
            "Completa: Al final, ponemos en la ensalada el zumo de 2 ...",

        ],
        respuestas1 : [
            "Solo la cáscara",
            "Cáscara y semillas"
        ],
        respuestas2 : [
            "Naranjas",
        ]
    },

    {
        nombre: "Pancakes",
        ruta : "../../ar-kids-pwa/assets/video/pancakes.mp4",
        checkpoints : [33, 57],
        preguntas : [
            "¿Cuántos minutos dejamos reposar la mezcla?",
            "Completa: Para dar la vuelta a la tortita usamos una ...",

        ],
        respuestas1 : [
            "5",
            "10"
        ],
        respuestas2 : [
            "Espátula",
        ]
    },   

    {
        nombre: "Sanduche",
        ruta : "../../ar-kids-pwa/assets/video/sandwich.mp4",
        checkpoints : [32, 96],
        preguntas : [
            "¿Que ingredientes tenemos que lavar?",
            "Completa: Este sandwich se llama ....",

        ],
        respuestas1 : [
            "Lechuga y tomate",
            "Ninguno"
        ],
        respuestas2 : [
            "Primavera"
        ]
    },     

    {
        nombre: "Sopa",
        ruta : "../../ar-kids-pwa/assets/video/sopa.mp4",
        checkpoints : [62, 126],
        preguntas : [
            "¿Cuántos minutos dejamos cocinar estas verduras?",
            "Completa: Acabamos de aprender la receta de una ....",

        ],
        respuestas1 : [
            "10",
            "2"
        ],
        respuestas2 : [
            "Sopa"
        ]
    },        

    {
        nombre: "Pollo",
        ruta : "../../ar-kids-pwa/assets/video/tortas pollo.mp4",
        checkpoints : [22, 119],
        preguntas : [
            "¿Cuántas libras de pechuga de pollo necesitamos",
            "Completa: Damos la vuelta a las tortitas cuando estén ....",

        ],
        respuestas1 : [
            "Media libra",
            "2 libras"
        ],
        respuestas2 : [
            "Doradas"
        ]
    },   
]

export const HOGAR_VIDEOS : Video[] = [
    {
        nombre: "Barrer",
        ruta : "../../ar-kids-pwa/assets/video/barrer.mp4",
        checkpoints : [43, 70],
        preguntas : [
            "¿Como se llama cuando eliminamos el detergente con agua?",
            "Completa: El secado se realiza con paños limpios o al .... "
        ],
        respuestas1 : [
            "Enjabonado",
            "Enjuagado"
        ],
        respuestas2 : [
            "Ambiente",
        ]
    },    
    
    {
        nombre: "Cama",
        ruta : "../../ar-kids-pwa/assets/video/cama.mp4",
        checkpoints : [32, 72],
        preguntas : [
            "¿Que retiramos primero para tender la cama?",
            "Completa: Para el frío del invierno podemos usar un ...",

        ],
        respuestas1 : [
            "Almohadas",
            "Cobijas"
        ],
        respuestas2 : [
            "Edredón",
        ]
    },

    {
        nombre: "Limpieza",
        ruta : "../../ar-kids-pwa/assets/video/limpieza.mp4",
        checkpoints : [58, 96],
        preguntas : [
            "¿Cómo limpiamos los cristales?",
            "Completa: Para sacar brillo a los cristales se usa ...",

        ],
        respuestas1 : [
            "De arriba hacia abajo",
            "De izquierda a derecha"
        ],
        respuestas2 : [
            "Papel",
        ]
    },   

    {
        nombre: "Mesa",
        ruta : "../../ar-kids-pwa/assets/video/mesa.mp4",
        checkpoints : [36, 172],
        preguntas : [
            "¿Que se pone primero para servir la mesa?",
            "Completa: La servilleta va de lado del ....",

        ],
        respuestas1 : [
            "Vaso",
            "Plato base"
        ],
        respuestas2 : [
            "Tenedor"
        ]
    },     

    {
        nombre: "Platos",
        ruta : "../../ar-kids-pwa/assets/video/platos.mp4",
        checkpoints : [44, 150],
        preguntas : [
            "¿Con que lavamos cada pieza?",
            "Completa: El cuchillo debe lavarse con cuidado, sujetando el ....",

        ],
        respuestas1 : [
            "Con una esponja o trapo limpio",
            "Solo con agua"
        ],
        respuestas2 : [
            "Agarrador"
        ]
    },        

    {
        nombre: "Reciclar",
        ruta : "../../ar-kids-pwa/assets/video/reciclar.mp4",
        checkpoints : [141, 252],
        preguntas : [
            "¿Podemos colocar botellas de mermelada en el basurero azul?",
            "Completa: Cuidar nuestro planeta es responsabilidad de ....",

        ],
        respuestas1 : [
            "Si",
            "No"
        ],
        respuestas2 : [
            "Todos"
        ]
    },   
]